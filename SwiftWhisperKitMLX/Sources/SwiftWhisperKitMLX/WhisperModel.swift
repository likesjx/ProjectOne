...
// Real MLX implementation guarded behind feature flag + availability.
#if USE_MLX && canImport(MLX)
import MLX
import MLXNN
import MLXRandom
#endif
...
#if !(USE_MLX) || !(canImport(MLX))
// Fallback shim so the package builds without MLX; all model ops unavailable.
public final class WhisperWeights: @unchecked Sendable { public let config: WhisperConfig; public let vocab:[String]; public init(config:WhisperConfig, vocab:[String]) { self.config=config; self.vocab=vocab } }
public enum WhisperLoader { public static func load(variant:WhisperVariant, preferredPaths:[URL]) throws -> WhisperWeights { throw WhisperModelError.modelUnavailable } }
#endif
