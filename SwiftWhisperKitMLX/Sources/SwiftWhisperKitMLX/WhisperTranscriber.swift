...
public final class MLXWhisperTranscriber: @unchecked Sendable {
    ...
    public func prepare() async throws {
        ...
        if let modelDir = paths.compactMap({ $0.appendingPathComponent(options.variant.raw) }).first(where: { FileManager.default.fileExists(atPath: $0.path) }) {
            ...
            if FileManager.default.fileExists(atPath: vocabURL.path), FileManager.default.fileExists(atPath: mergesURL.path) { tokenizer = try? WhisperTokenizer(vocabURL: vocabURL, mergesURL: mergesURL) }
        }
        prepared = true
        #else
    ...
    public func transcribe(samples: [Float], sampleRate: Double) async throws -> WhisperTranscribeResult {
        ...
        let text: String = (tokenizer?.decode(tokens)) ?? tokens.compactMap { $0 < w.vocab.count ? w.vocab[$0] : nil }.joined(separator: " ")
