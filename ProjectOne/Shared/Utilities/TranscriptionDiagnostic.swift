...
public class TranscriptionDiagnostic {
    ...
    private func checkMicrophonePermission() async -> CheckResult {
        ...
        #if os(iOS)
        // Use modern permission checking approach for iOS 17.0+
        let hasPermission: Bool
        
        do {
            // Test microphone access by trying to create a test recorder
            let tempURL = FileManager.default.temporaryDirectory.appendingPathComponent("permission_test_\(UUID().uuidString).wav")
            ...
            hasPermission = testRecorder.prepareToRecord()
            
            // Clean up test file
            try? FileManager.default.removeItem(at: tempURL)
            
        } catch {
            logger.warning("Could not test microphone permission")
            hasPermission = false
        }
        ...
        if !hasPermission {
            issues.append(DiagnosticIssue(
                ...
                description: "Microphone access appears to be denied or unavailable",
                technicalDetails: "Test recorder could not prepare to record"
            ))
            recommendations.append("Guide user to Settings > Privacy & Security > Microphone to enable access")
        } else {
            logger.info("✅ Microphone permission appears to be granted")
        }
        #else
    ...
    private func checkEngineSelection() async -> CheckResult {
        ...
        var recommendations: [String] = []
        do {
            let factory = await SpeechEngineFactory()
    ...
    private func checkAudioFormatCompatibility() -> CheckResult {
        ...
        let sampleRate = audioSettings[AVSampleRateKey]!
        if sampleRate < 16000 {
            issues.append(DiagnosticIssue(
                type: .audioFormat,
                severity: .warning,
