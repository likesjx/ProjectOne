...
public class WhisperKitTranscriber: NSObject, SpeechTranscriptionProtocol, @unchecked Sendable {
    ...
    public func prepare() async throws {
        ...
        // Skip preloaded WhisperKit to avoid Sendable issues
        // Note: preloaded model functionality disabled
        
        // Fall back to progressive model loading
        ...
        for (index, fallbackModelSize) in fallbackModels.enumerated() {
            ...
            let task = Task { @Sendable in
                do {
                    ...
                    logger.error("Error domain: \(error._domain)")
                    logger.error("Error code: \(error._code)")
                    logger.error("Error userInfo: \(String(describing: error))")
                    throw error
                }
            }
            ...
            // Capture the task to avoid closure capture issues
            let capturedTask = task
            
                whisperKit = try await withThrowingTaskGroup(of: WhisperKit?.self) { group in
                    group.addTask { @Sendable in 
                        return try await capturedTask.value 
                    }
    ...
    public func transcribe(audio: AudioData, configuration: TranscriptionConfiguration) async throws -> SpeechTranscriptionResult {
        ...
        do {
            ...
            let transcriptionTask = Task { @Sendable in
                do {
            ...
            // Capture the task to avoid closure capture issues
            let capturedTranscriptionTask = transcriptionTask
            ...
            let transcriptionResult = try await withThrowingTaskGroup(of: TranscriptionSegment.self) { group in
                group.addTask { @Sendable in
                    let result = try await capturedTranscriptionTask.value
    ...
    public func transcribeRealTime(audioStream: AsyncStream<AudioData>, configuration: TranscriptionConfiguration) -> AsyncStream<SpeechTranscriptionResult> {
        ...
        return AsyncStream { continuation in
            ...
            // Capture configuration, locale, and methods to avoid data races
            let capturedConfiguration = configuration
            let capturedLocale = locale
            let getWhisperLanguageCodeMethod = self.getWhisperLanguageCode
            let logger = self.logger
            let createTranscriptionResultMethod = self.createTranscriptionResult
            ...
            Task { [weak self] in
                do {
                    ...
                    for await audioData in audioStream {
                        ...
                        let samples = try self.convertAudioDataToSamples(audioData)
                        ...
                        if audioBuffer.count >= bufferSizeFrames {
                            ...
                            let options = DecodingOptions(
                                task: capturedConfiguration.enableTranslation ? .translate : .transcribe,
                                language: getWhisperLanguageCodeMethod(capturedConfiguration.language ?? capturedLocale.identifier),
                                temperature: 0.0,
                            ...
                            if let result = results.first {
                                let transcriptionResult = createTranscriptionResultMethod(
                                    result,
                                    0.0, // Real-time processing
                    ...
                    if !audioBuffer.isEmpty && audioBuffer.count > Int(sampleRate) { // At least 1 second
                        ...
                        let finalOptions = DecodingOptions(
                            task: configuration.enableTranslation ? .translate : .transcribe,
                            language: getWhisperLanguageCodeMethod(configuration.language ?? capturedLocale.identifier),
                            temperature: 0.0,
                        ...
                        if let result = results.first, let strongSelf = self {
                            let transcriptionResult = strongSelf.createTranscriptionResult(
                                from: result,
                                processingTime: 0.0,
                                configuration: configuration
                            )
                    ...
                    logger.error("Real-time WhisperKit transcription error: \(error.localizedDescription)")
                    continuation.finish()
                }
            }
            #endif
