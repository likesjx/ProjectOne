...
public class MemoryAgentModelProvider: Sendable {
    ...
    public func generateResponse(prompt: String, context: MemoryContext) async throws -> AIModelResponse {
        // 1. Optimize and validate context (run on main actor)
        ...
        // 4. Post-process response (run on main actor)
    ...
    private func generateWithFallback(
        ...
        if #available(iOS 26.0, macOS 26.0, *), let appleProvider = appleFoundationProvider {
            ...
            do {
                ...
                // Capture context manager locally to avoid data races
                let contextManager = self.contextManager
                let responseProcessor = self.responseProcessor
                
                let response = try await withTimeout(configuration.responseTimeoutInterval) { @Sendable in
                    return try await self.generateWithAppleFoundation(prompt: prompt, context: context, provider: appleProvider, contextManager: contextManager, responseProcessor: responseProcessor)
                }
        ...
        if let factory = externalProviderFactory {
            ...
            for provider in activeProviders {
                ...
                do {
                    // Capture references locally to avoid data races
                    let contextManager = self.contextManager
                    let responseProcessor = self.responseProcessor
                    
                    let response = try await withTimeout(configuration.responseTimeoutInterval) { @Sendable in
                        return try await self.generateWithExternalProvider(prompt: prompt, context: context, provider: provider, contextManager: contextManager, responseProcessor: responseProcessor)
                    }
...
public class MemoryContextManager: @unchecked Sendable {
...
public class AIProviderSelector: @unchecked Sendable {
...
public class ResponseProcessor: @unchecked Sendable {
